...
var common_tpl ='<section class="collapse   section--full" id="cat-nav">' +
                '<ul role="navigation" class="section-nav list-unstyled">' +
                    '<li><a class="menu-advisory" data-toggle="collapse" href="#collapse-advisory" aria-expanded="false" aria-controls="collapse-advisory">養老保障機制</a>' +
                        ...
                        '</div>' +
                    '</li>' +
                    '<li><a class="menu-education" data-toggle="collapse" href="#collapse-education" aria-expanded="false" aria-controls="collapse-education">學習</a>' +
                        '<div class="collapse" id="collapse-education">' +
                            '<ul class="children menu-education list-unstyled">' +
                                '<li><a href="education.html?slug=education/courses" data-reload="true" data-ignore-cache="true">課程資訊</a></li>' +
                                '<li><a href="education.html?slug=education/tips" data-reload="true" data-ignore-cache="true">學習小貼士</a></li>' +
                                '<li><a href="education.html?slug=education/edcontact" data-reload="true" data-ignore-cache="true">通訊錄</a></li>' +
                                '<li><a href="education.html?slug=education/learningmag" data-reload="true" data-ignore-cache="true">終身學習雜誌</a></li>' +
                            '</ul>' +
                        '</div>' +
                    '</li>' +
                    ...
                    '<li><a class="menu-service" data-toggle="collapse" href="#collapse-service" aria-expanded="false" aria-controls="collapse-service">服務</a>' +
                        '<div class="collapse" id="collapse-service">' +
                            '<ul class="children menu-service list-unstyled">' +
                                '<li><a href="service.html?slug=service/healthcare" data-reload="true" data-ignore-cache="true">健康照顧</a></li>' +
                                '<li><a href="service.html?slug=service/economics" data-reload="true" data-ignore-cache="true">經濟保障</a></li>' +
                                '<li><a href="service.html?slug=service/housing" data-reload="true" data-ignore-cache="true">房屋政策</a></li>' +
                                '<li><a href="service.html?slug=service/entertainment" data-reload="true" data-ignore-cache="true">文娛康體</a></li>' +
                                '<li><a href="service.html?slug=service/living" data-reload="true" data-ignore-cache="true">日常生活</a></li>' +
                                '<li><a href="service.html?slug=service/economics" data-reload="true" data-ignore-cache="true">其他</a></li>' +
                            '</ul>' +
                        '</div>' +
                    '</li>' +
                    ...
                    '<li><a class="menu-news" data-toggle="collapse" href="#collapse-news" aria-expanded="false" aria-controls="collapse-news">新聞</a>' +
                        '<div class="collapse" id="collapse-news">' +
                            '<ul class="children menu-news list-unstyled">' +
                                '<li><a href="news.html?slug=news/announcement" data-reload="true" data-ignore-cache="true">特别消息</a></li>' +
                                '<li><a href="news.html?slug=news/events" data-reload="true" data-ignore-cache="true">活動訊息</a></li>' +
                                '<li><a href="news.html?slug=news/welfare" data-reload="true" data-ignore-cache="true">福利政策資訊</a></li>'
                            '</ul>' +
                       '</div>' +
                    '</li>' +
...
$$(document).on('pageInit', function (e) {
  ...
  // 所有页面都会用到的公共js 部分
    ...
    $('#cat-nav a[href="#collapse-'+page.name+'"]').trigger('click');
...
myApp.onPageInit('education', function (page) {
    $$("div#tpl_common_nav").html(common_html);

    var nav = $('#cat-nav a[href="'+location.hash.substr(3)+'"]');
    if(nav.length == 0) {
        nav = $('#cat-nav a[href^="'+location.hash.substr(3)+'"]').first();
    }
    var slug = page.query.slug !== undefined ? page.query.slug : nav.attr('href').split('slug=')[1];
    ajaxServiceSlug = '/api/' + slug;

    $('.page-header').text(nav.text());

    $$.ajax({
        url: ajaxDomain + ajaxServiceSlug + '/categories',
        type: 'GET',
        success: function (getData) {
            // 转换Json数据成对象
            var a = JSON.stringify(getData);
            ...
            $.each(data.list,function(index, value){
                $('.filter__option').append('<li><a class="filter__option__item'+(value.slug ==slug ? ' active' : '')+'" href="'+page.name+'.html?slug='+value.slug+'" data-reload="true" data-ignore-cache="true">'+value.name+'</a></li>');
                if (value.slug == slug) {
                    $('.page-header').text(value.name);
                }
            });
        }
    });
    ...
    $$.ajax({
        url: ajaxDomain + ajaxServiceSlug + '?page=1&pn=10',
        type: 'GET',
        async:false,
        success: function (getData) {
            // 转换Json数据成对象
...
//  无限滚动  //
    ...
    var loading = false;
    // Last loaded index
    var lastIndex = $$('#tpl_edu .post-list li').length;
    // 最多可加载的条目
    var maxItems = 60;
    ...
    $$('.infinite-scroll').on('infinite', function () {
        ...
        setTimeout(function () {
            ...
            $$.getJSON( ajaxDomain + ajaxServiceSlug + '?page='+page+'&pn='+itemsPerLoad,function(data){
                if(data.length == 0) {
                    myApp.detachInfiniteScroll($$('.infinite-scroll'));
                    // Remove preloader  // 删除加载提示符
                    $$('.infinite-scroll-preloader').remove();
...
//  无限滚动  //
    ...
    var loading = false;
    // Last loaded index
    var lastIndex = $$('#tpl_news .post-list li').length;
    // 最多可加载的条目
    var maxItems = 60;
    // 每次加载添加多少条目 Append items per load
    var itemsPerLoad = 10;
    ...
    $$('.infinite-scroll').on('infinite', function () {
        ...
        setTimeout(function () {
            ...
            $$.getJSON( ajaxDomain + ajaxServiceSlug + '?page='+page+'&pn='+itemsPerLoad,function(data){
                ...
                for (var i = 0; i < itemsPerLoad; i++) {
                    var it = data[i];
                    $$('.post-list').append('<li class="post-list__item"><a class="J_link" href="'+ajaxDomain+'/api/news/detail/'+it.id+'"><span class="post-list__item__title">'+it.title+'</span><span class="post-list__item__date">'+it.start_at+'</span></a></li>');
                }
...
myApp.onPageInit('service', function (page) {
    ...
    $$.ajax({
        url: ajaxDomain + ajaxServiceSlug + '?page=1&pn=3', //ajaxHealthTips   localhost:8000/api/service/healthcare
        type: 'GET',
        success: function (getData) {
            // 转换Json数据成对象
            var a = JSON.stringify(getData);
